...
const body = z.discriminatedUnion("decision", [
  ...
  z.object({ processedEmailId: uuid, decision: z.literal("rejected") }),
]);
...
// values) and any decision already saved.
export async function GET(request: Request) {
  ...
  const failure = fields.error ?? resolution.error;
  ...
  const defaults = fields.data
    ...
    : null;
  return NextResponse.json({ success: true, defaults, resolution: resolution.data });
}
...
// reason) and then records the decision.
export async function POST(request: Request) {
  ...
  if (input.decision === "rejected") {
    const { data: processed } = await supabase
      ...
      .select("email_id, review_reason")
      .eq("id", input.processedEmailId)
      .maybeSingle<{ email_id: string; review_reason: string | null }>();
    ...
    const { data: existing } = await supabase.from("review_resolutions").select("decision").eq("processed_email_id", input.processedEmailId).maybeSingle();
    if (existing?.decision === "rejected") {
      return NextResponse.json({ success: false, error: "A rejection reply has already been sent for this email." }, { status: 409 });
    }
    ...
    const reply = replyContent({ from: email.from_address, subject: email.subject, reason: processed.review_reason });
    try {
